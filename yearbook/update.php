...
try {
    ...
    if ($_POST['passwd'] != '') {
        ...
        $SQLUpdate = 'UPDATE  participantes SET  login = ? , senha =  MD5(?), nomeCompleto = ?,  arquivoFoto = ?, cidade =  ?, email = ?,
							  descricao = ? WHERE  nomeCompleto= ?';
        //prepara a execução
        $operacao = $conexao->prepare($SQLUpdate);

        //executa a sentença SQL com os parâmetros passados por um vetor
        $inserir = $operacao->execute(array($login, $senha, $nome, $pathCompleto, $cidade, $email, $descricao, $_SESSION['nomeCompleto']));
    ...
    if ($inserir) {
        ...
        echo "<p class=\"lead\"><a href=\"./pag_principal.php\">Página Principal</a></p>\n";
    }
    else {
